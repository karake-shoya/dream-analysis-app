---
name: seo-analyze
description: Yume Insight（dream-analysis-app）のSEO分析のみを実行する。データ取得→過去PR確認→分析・優先度付き改善案の提示まで行う。修正・PR作成は行わない。「SEO分析だけ」「分析のみ」「seo-analyze」などと言われたとき使用。
---

# Yume Insight SEO分析（分析のみ）

修正・PRは行わず、分析と改善案の提示に特化したスキル。  
修正まで実行したい場合は `/seo-run` を使うこと。
...
```bash
gh pr list --search "SEO 週次" --base main --state all --limit 5 --json number,title,mergedAt,files
```

取得したファイルパス一覧を記録しておき、ステップ2の分析時に除外する。

### ステップ2：データを読み込んでSEO分析・提示
...
分析結果と改善案（優先度付き）を日本語で提示して終了。  
修正を実行したい場合は `/seo-run` を使うよう案内する。

## 注意事項

- 修正・コミット・PRは行わない
- Gemini APIは使用しない（Claudeが直接分析・判断する）
