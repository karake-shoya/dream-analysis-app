import { Palette, BookOpen, Brain, Eye } from 'lucide-react';
import ColumnArticleShell from '@/components/column/ColumnArticleShell';
import InContentAd from '@/components/InContentAd';
import { buildColumnMetadata } from '@/lib/seo';

const FAQS = [
  { q: '夢はカラーで見るものですか、白黒で見るものですか？', a: '研究によると、現代人の多くはカラーで夢を見ています。1940〜50年代はモノクロ夢の報告が多かったですが、カラーテレビの普及とともにカラー夢が増加しました。どちらが普通ということはなく、睡眠のタイミングや夢を想起するタイミングによっても異なります。' },
  { q: '夢の中でよく見る色に心理的な意味はありますか？', a: 'ユング心理学や色彩心理学では、夢に繰り返し現れる色は現在の感情状態や無意識のテーマを象徴していると考えられています。ただし色の象徴は文化や個人的な経験によって異なるため、「その色がどう感じられたか」という主観的な印象も大切にしながら解釈することが重要です。' },
  { q: '夢の中で赤いものを見たのですが、悪い意味がありますか？', a: '必ずしも悪い意味ではありません。赤は情熱・生命力・怒りなど強度の高い感情を象徴しますが、夢の中でどう感じたかによって解釈が変わります。温かく感じた赤なら活力や自信の回復を、怖かった赤なら緊張や抑えきれない感情を映しているかもしれません。' },
  { q: '夢の色を覚えていないのですが問題ありますか？', a: '問題ありません。夢の細部（色・音・触感など）は特に記憶に残りにくいものです。目覚めた直後に「印象的な色はあったか？」と意識的に問いかける習慣をつけると、少しずつ気づきやすくなります。夢日記に「色」の記録欄を設けると効果的です。' },
  { q: '夢の色と現実の気分に関係はありますか？', a: '心理学的には、感情状態が夢の内容（色を含む）に影響するとされています。ストレスが高い時期には赤や黒などの強い色が印象に残りやすく、気持ちが安定しているときには青や緑の穏やかな色が現れやすいという傾向があります。夢日記で色を記録すると、自分の感情サイクルと夢の色の相関が見えてくることがあります。' },
];

export const metadata = buildColumnMetadata('dream-color');

export default function DreamColorPage() {
  return (
    <ColumnArticleShell
      slug="dream-color"
      heroTitle="夢の中の色が持つ心理的な意味"
      heroSubtitle="印象に残った色は、心の深層からのメッセージかもしれない"
      faqs={FAQS}
      faqVariant="qa"
      disclaimer="※本ページの内容は、心理学・色彩心理学の一般知識をもとにした情報提供を目的としており、医学的な診断・治療を行うものではありません。心の不調が続く場合は、専門家にご相談ください。"
      cta={{
        title: '夢の色が気になるときは',
