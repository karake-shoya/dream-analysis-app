...
const FAQS = [
  { q: '怖い夢を見た朝はどうすればいいですか？', a: '内容と感情を簡単に書き留めてみてください。書くことで夢を客観視する距離ができ、怖さが和らぎます。ユング心理学的には「夢の怖い存在が何を象徴しているか」を考えることで、深層心理のサインとして受け取ることができます。' },
  { q: '悪夢は病気のサインですか？', a: '単発の悪夢はストレスや疲労の反応として誰でも起こります。ただし、同じ悪夢が繰り返されたり、日常生活に支障が出るほど頻繁な場合は、PTSDや睡眠障害の可能性もあります。その場合は睡眠専門医や心療内科への相談を検討してください。' },
  { q: '悪夢を繰り返し見るのはなぜですか？', a: 'ユング心理学では、繰り返す悪夢は「シャドウ（無意識に抑圧された感情）が意識化を強く求めている状態」として解釈されます。向き合えていない感情やテーマが解消されないまま残っているため、夢が繰り返されます。そのテーマに意識的に向き合えたとき、繰り返しの悪夢が自然に止まることがあります。' },
];

export const metadata = buildColumnMetadata('nightmare');

export default function NightmarePage() {
  return (
    <ColumnArticleShell
      slug="nightmare"
      heroTitle="怖い夢・悪夢を見やすい人の特徴と対処法"
      heroSubtitle="悪夢が続くとき、心は何かを伝えようとしている"
      faqs={FAQS}
      references={[
