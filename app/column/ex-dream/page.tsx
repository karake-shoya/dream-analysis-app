...
const FAQS = [
  { q: '元カレ・元カノが夢に出てくるのはまだ好きだからですか？', a: '必ずしもそうではありません。ユング心理学では、夢に登場する元恋人は「その人自身」ではなく、その人が象徴する感情・関係パターン・未解決の心理テーマを反映していることが多いとされています。未練がなくても、関係から学んだ何かが夢として現れることがあります。' },
  { q: '別れて何年も経つのに元恋人が夢に出てくるのはなぜですか？', a: '時間が経っても夢に出てくる場合、「その元恋人との関係から生まれた感情パターン」が現在の自分の中でまだ生きているサインかもしれません。特に、現在の人間関係や感情面で似たようなテーマ（孤独・信頼・見捨てられ恐怖など）が再び動いているときに、過去の象徴として現れることがあります。' },
  { q: '夢で元恋人と仲直りする・抱き合う夢はどんな意味ですか？', a: '元恋人と和解・再接触する夢は、その人への未練よりも「その関係で傷ついた部分が癒え始めている」または「自分の中の何かを受け入れようとしている」プロセスを示すことがあります。ユング的には、自分の中のアニマ・アニムスとの和解として解釈されることもあります。' },
  { q: '元恋人が夢に出てきた後、連絡してもいいですか？', a: '夢は「連絡すべきかどうか」を教えてくれるものではありません。夢の中の相手は自分の深層心理が作り出した象徴です。連絡を取るかどうかは、夢ではなく現実の状況と自分の意思で判断してください。夢を「衝動の根拠」にするのは避けることをおすすめします。' },
  { q: '今のパートナーがいるのに元恋人の夢を見るのは罪悪感を感じてしまいます', a: '罪悪感を持つ必要はありません。夢は意識的に選べるものではなく、深層心理の自然な処理プロセスです。今のパートナーへの愛情と、過去の記憶が夢に出ることは矛盾しません。大切なのは夢の内容ではなく、目覚めているときの現実の行動と気持ちです。' },
];

export const metadata = buildColumnMetadata('ex-dream');

export default function ExDreamPage() {
  return (
    <ColumnArticleShell
      slug="ex-dream"
      heroTitle="元カレ・元カノが夢に出てくる意味とは"
      heroSubtitle="未練だけではない——深層心理が映し出す過去との対話"
      faqs={FAQS}
      references={[
        ...
        <>Sigmund Freud, <em>The Interpretation of Dreams</em></>,
      ]}
      disclaimer="※本ページの内容は、心理学的な一般知識をもとにした情報提供を目的としており、医学的な診断・治療を行うものではありません。心理的なサポートが必要と感じる場合は、専門家にご相談ください。"
      nextColumn={{
        href: '/column/dream-self-care',
        emoji: '💜',
        heading: '夢をセルフケアに活かす方法も読んでみませんか？',
        sub: '悪夢・繰り返す夢の心理的な向き合い方',
      }}
      cta={{
