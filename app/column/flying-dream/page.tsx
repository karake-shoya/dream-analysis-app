...
const FAQS = [
  { q: '空を飛ぶ夢は良い夢ですか？', a: '一般的に「吉夢」として解釈されることが多い夢です。自由・解放・自信・上昇志向の高まりを象徴することが多く、目覚めたときに爽快感がある場合は特にポジティブなサインと受け取れます。ただし飛ぶのが怖い・うまく飛べないなどのシーンでは、不安や自信のなさを反映していることもあります。' },
  { q: '飛べなくて落ちそうになる夢はどんな意味ですか？', a: '飛んでいる途中で高度が下がる・落ちそうになる夢は、自信や意欲が持続しにくい状態、または目標に向かう途中で不安が出てきていることを反映している可能性があります。「このままうまくやれるだろうか」という不安が夢に現れていることがあります。' },
  { q: '繰り返し空を飛ぶ夢を見るのはなぜですか？', a: 'ユング心理学では、繰り返す夢は深層心理が特定のテーマに注目しているサインです。空を飛ぶ夢が繰り返される場合、「自由への強い欲求」「現実の制約から解放されたい気持ち」が続いている状態を反映していることが多いです。現実でどんな制約や重圧を感じているか、振り返ってみると手がかりが見つかります。' },
  { q: '誰かと一緒に飛ぶ夢と一人で飛ぶ夢では意味が違いますか？', a: '一人で飛ぶ夢は、自分自身の力・自律性・独立心の高まりを、誰かと一緒に飛ぶ夢はその人との関係における安心感・信頼・共同の目標意識を反映することがあります。誰と飛んだかも、夢の意味を読み解く手がかりになります。' },
  { q: '空を飛ぶ夢と明晰夢は関係がありますか？', a: '空を飛ぶ夢は、明晰夢（夢の中で「夢だと気づいている状態」）で最も多く報告される体験のひとつです。飛行中に「あ、これは夢だ」と気づくことで明晰夢に移行するケースも多く、夢のコントロールを学ぶ入口として活用されることがあります。' },
];

export const metadata = buildColumnMetadata('flying-dream');

export default function FlyingDreamPage() {
  return (
    <ColumnArticleShell
      slug="flying-dream"
      heroTitle="空を飛ぶ夢の意味とは"
      heroSubtitle="自由・解放・上昇——深層心理が求めているもの"
      faqs={FAQS}
      references={[
      ...
      nextColumn={{
        ...
        emoji: '🌙',
        heading: '夢の中で「夢だ」と気づく体験に興味がありますか？',
        sub: '明晰夢（ルシッドドリーム）とは？見る方法・練習法をユング心理学で解説',
      }}
      cta={{
