...
const FAQS = [
  { q: '遅刻する夢はどんな意味がありますか？', a: '遅刻する夢は、チャンスを逃す恐れ・準備不足への不安・「乗り遅れている」という感覚を反映することが多いです。現実で締め切り・重要な機会・プレッシャーを感じているとき、またはより慢性的な「時間に追われている」感覚があるときに出やすい夢です。' },
  { q: '遅刻の夢を繰り返し見るのはなぜですか？', a: '遅刻の夢を繰り返し見る場合、慢性的なプレッシャー・時間への焦り・「間に合わないのではないか」という不安が解消されていない状態を反映しています。現実の生活でペース配分・優先順位・自分への期待値を見直す必要があるサインとして受け取れます。' },
  { q: '遅刻しそうで焦っているのに動けない夢はどんな意味ですか？', a: '急ぎたいのに体が動かない・走れないという夢は、焦りとは裏腹に前進できない無力感を象徴します。どれだけ努力しても追いつかないという感覚や、状況に対して主体性を発揮できないという深層の感覚を反映しています。' },
  { q: '試験・大事な予定に遅刻する夢はどんな意味ですか？', a: '試験・大事な発表・面接などに遅刻する夢は、その場面で「準備ができていない・評価される機会を失う」ことへの不安を直接的に反映しています。完璧主義・自己評価の低さ・重要な機会を前にしたプレッシャーが大きいときに特に出やすいパターンです。' },
  { q: '遅刻したのに誰も気にしていない夢はどんな意味ですか？', a: '遅刻しても誰も気にしていない夢は、「自分が思うほど他者は自分のことを注意していない」という気づきを象徴します。過度な自意識・「失敗したら大変なことになる」という思い込みが和らいでいるサインとして解釈できます。' },
];

export const metadata = buildColumnMetadata('late-dream');

export default function LateDreamPage() {
  return (
    <ColumnArticleShell
      slug="late-dream"
      heroTitle="遅刻する夢の意味とは"
      heroSubtitle="焦り・準備不足・時間へのプレッシャー——深層心理が映すもの"
      faqs={FAQS}
      references={[
      ...
      nextColumn={{
        href: '/column/school-dream',
        emoji: '🏫',
        heading: '学校の夢も気になりますか？',
        sub: '試練・評価・帰属——学校が映す深層心理',
      }}
      cta={{
        title: '遅刻する夢をより詳しく分析したい方へ',
        description: '夢の詳細をAIに伝えると、あなたの深層心理をより具体的に読み解きます。どんな場面・何に遅刻したか・感じた感情も含めて入力してみてください。',
      }}
    >
