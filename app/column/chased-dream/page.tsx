...
const FAQS = [
  { q: '追いかけられる夢は悪い意味がありますか？', a: '必ずしも悪い意味ではありません。追いかけられる夢は、向き合えていない感情・課題・ストレスが無意識から意識化を求めているサインとして解釈されます。怖い体験ではありますが、「今の自分が何を避けているか」に気づくきっかけになることがあります。' },
  { q: '追いかけてくる存在が人間の場合と怪物の場合では意味が違いますか？', a: 'ユング心理学的には、どちらもシャドウ（抑圧された感情や自己像）の象徴ですが、人間の場合は現実の人間関係や具体的な状況との関連が強く、怪物・正体不明の存在の場合はより深い無意識のテーマ（漠然とした恐怖・実体のない不安）を反映しやすいと言われています。' },
  { q: '夢の中で逃げきれた場合と捕まった場合では意味が違いますか？', a: '逃げきれた場合は、現実でも回避や先送りが続いている状態を、捕まった場合は「もう逃げられない」という限界点・向き合わざるを得ない局面に来ていることを示す可能性があります。ただし捕まった後の感情（恐怖・安堵・諦め）も解釈の重要な手がかりです。' },
  { q: '追いかけられる夢を繰り返し見るのはなぜですか？', a: 'ユング心理学では、繰り返す夢はその象徴するテーマが未解決のまま残っているサインです。回避し続けている感情・状況・人間関係が解消されないため、無意識が繰り返し意識化を求めています。そのテーマに意識的に向き合えたとき、繰り返しが止まることがあります。' },
  { q: '追いかけられる夢を見た後にできることはありますか？', a: '目覚めたとき感じた感情（恐怖・焦り・疲労感など）と、追いかけてきた存在の特徴を短くメモしておきましょう。次に「最近何を先送りしているか」「向き合うのが怖いことは何か」を静かに問いかけてみてください。夢日記に記録し続けると、パターンが見えてきます。' },
];

export const metadata = buildColumnMetadata('chased-dream');

export default function ChasedDreamPage() {
  return (
    <ColumnArticleShell
      slug="chased-dream"
      heroTitle="追いかけられる夢の意味とは"
      heroSubtitle="逃げ続けるとき、深層心理は何を伝えようとしているのか"
      faqs={FAQS}
      references={[
      ...
      nextColumn={{
        ...
        emoji: '🦷',
        heading: '歯が抜ける夢も気になりますか？',
        sub: '自信・変化・コミュニケーション——深層心理が映し出すもの',
      }}
      cta={{
