...
const FAQS = [
  { q: '知らない家が夢に出てくるのはなぜですか？', a: 'ユング心理学では「家」は自己（Self）や精神の全体像を象徴します。知らない家が出てくるのは、まだ自分が探求していない内面の領域があることを示すことが多いです。新しい自己の側面・まだ気づいていない可能性・未知の感情が、知らない家として現れます。' },
  { q: '夢に出てくる家の部屋の数や広さに意味がありますか？', a: 'ユング心理学的には、家の広さや部屋の数は内面の豊かさや複雑さを象徴することがあります。広い豪華な家は精神的な可能性の広がりを、狭く閉塞した家は圧迫感・制限を感じている状態を反映することがあります。' },
  { q: '夢の中の家が崩れる・壊れる場合はどんな意味ですか？', a: '家が崩れる夢は、自己の基盤となっている何か（価値観・アイデンティティ・関係・生活基盤）が揺らいでいる状態を反映することがあります。大きな変化の前に見やすい夢で、必ずしも悪い意味ではなく「古い構造が壊れて新しいものが始まる」変容の過程として解釈されることもあります。' },
  { q: '夢の中で家の中に隠し部屋や秘密の空間を見つける夢はどんな意味ですか？', a: 'これはユング的に非常に興味深い夢です。まだ探索していない自己の側面・隠れた才能・抑圧されていた感情が「発見」を待っている状態を象徴します。この夢を見た後は「まだ自分が気づいていない自分の一面はないか」を問いかけてみましょう。' },
  { q: '毎回同じ知らない家が夢に出てくるのはなぜですか？', a: '繰り返し同じ家が出てくる夢は、その家が象徴する内面のテーマが未解決のまま残っているサインです。その家でどんな感情を感じるか・どんなことが起きるかが、未解消のテーマへの手がかりになります。' },
];

export const metadata = buildColumnMetadata('unknown-house-dream');

export default function UnknownHouseDreamPage() {
  return (
    <ColumnArticleShell
      slug="unknown-house-dream"
      heroTitle="知らない家・部屋が出てくる夢の意味とは"
      heroSubtitle="家は自己の鏡——内面の探索が始まるとき"
      faqs={FAQS}
      references={[
      ...
      cta={{
        title: '知らない家の夢をより詳しく分析したい方へ',
        description: '夢の詳細をAIに伝えると、あなたの深層心理をより具体的に読み解きます。家の様子や探索した場所、感じた感情も含めて入力してみてください。',
      }}
    >
