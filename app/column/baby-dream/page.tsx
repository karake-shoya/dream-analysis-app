import { Heart, Brain, Search, BookOpen } from 'lucide-react';
import ColumnArticleShell from '@/components/column/ColumnArticleShell';
import InContentAd from '@/components/InContentAd';
import { buildColumnMetadata } from '@/lib/seo';

const FAQS = [
  { q: '赤ちゃんが出てくる夢は良い夢ですか？', a: '一般的にポジティブな意味合いが多い夢です。新しい始まり・純粋さ・可能性・内なる自己の成長を象徴することが多く、目覚めたときに温かい感情があった場合は特にそのサインと受け取れます。ただし赤ちゃんの状態（泣いている・笑っている・危険な状況など）によって解釈が変わります。' },
  { q: '子どもがいないのに赤ちゃんを抱っこする夢を見るのはなぜですか？', a: 'ユング心理学では、赤ちゃんは「自分の内なる新しい可能性・育てる必要がある部分」を象徴します。実際の子どもの有無とは関係なく、「何か新しいことを始めたい・育てたい」という欲求や、自分の中の純粋な部分への関心が高まっているときに見やすい夢です。' },
  { q: '赤ちゃんが泣いている夢はどんな意味ですか？', a: '赤ちゃんが泣いている夢は、自分の中の「世話が必要な部分」が助けを求めているサインとして解釈されます。感情的なサポートの不足・自分のニーズを無視している状態・創造的なエネルギーが出口を求めているなどが考えられます。' },
  { q: '赤ちゃんが危険な状況にある夢はどんな意味ですか？', a: '赤ちゃんが危険にさらされる夢は、新しいプロジェクト・関係・アイデア（赤ちゃんが象徴するもの）が脅かされている不安を反映することがあります。大切なものを守れるかどうかへの不安が夢として現れています。' },
  { q: '自分が赤ちゃんに戻る夢はどんな意味ですか？', a: '自分が赤ちゃんや子どもに戻る夢は、無邪気さ・保護されたい気持ち・責任から解放されたい欲求を反映することがあります。プレッシャーが強い時期に、過去の安心感を求めて出やすい夢です。' },
];

export const metadata = buildColumnMetadata('baby-dream');

export default function BabyDreamPage() {
  return (
    <ColumnArticleShell
      slug="baby-dream"
      heroTitle="赤ちゃん・子供が出てくる夢の意味とは"
      heroSubtitle="新しい始まり・純粋さ・可能性——深層心理が育てるもの"
      faqs={FAQS}
      references={[
      ...
      nextColumn={{
        href: '/column/death-dream',
        emoji: '🌙',
        heading: '変容・再生のサインとしての死ぬ夢も読んでみませんか？',
        sub: '終わりではなく新しい始まりを告げる夢',
      }}
      cta={{
        title: '赤ちゃん・子どもが出てくる夢をより詳しく分析したい方へ',
        description: '夢の詳細をAIに伝えると、あなたの深層心理をより具体的に読み解きます。どんな状況だったか、感じた感情も含めて入力してみてください。',
      }}
    >
