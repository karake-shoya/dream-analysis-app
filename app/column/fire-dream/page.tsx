import { Flame, Search, BookOpen } from 'lucide-react';
import ColumnArticleShell from '@/components/column/ColumnArticleShell';
import InContentAd from '@/components/InContentAd';
import { buildColumnMetadata } from '@/lib/seo';

const FAQS = [
  { q: '火の夢は良い夢ですか？悪い夢ですか？', a: '火の夢はどちらとも決まりません。炎は破壊と創造の両面を持つシンボルです。穏やかな焚き火・ろうそくの炎は温もり・情熱・光を、制御不能な火事・燃え広がる炎は消費しきれないほどのエネルギー・怒り・急激な変化を象徴します。夢の中の火の状態と感情が解釈の鍵です。' },
  { q: '家が燃える夢はどんな意味ですか？', a: '家は自己・家族・安全基盤の象徴です。家が燃える夢は、自分の基盤や大切なものが失われることへの恐怖・または大きな変化によってこれまでの自分が変容していくプロセスを反映することがあります。破壊的変化の後に新しい自己が生まれるという再生のサインとして解釈されることもあります。' },
  { q: '火を見ている夢はどんな意味ですか？', a: '火を観察している夢は、情熱・欲求・変化を自分の内側で感じながら、まだそれと距離をおいている状態を示します。関わりたいが関わることへの迷いや、変化の入り口に立っている状態として解釈されます。' },
  { q: '火を消す夢はどんな意味ですか？', a: '情熱・怒り・感情の過剰なエネルギーを鎮めようとしている状態を反映します。感情をコントロールしようとする意志や、激しい状況を収束させようとする心の動きとして読み解けます。' },
  { q: 'ユング心理学で火はどんな象徴ですか？', a: 'ユング心理学では、火は変容・情熱・精神的エネルギー（リビドー）のシンボルです。錬金術における火は「変容の媒介」として重要な役割を持ちます。夢の中の火は、今の自分を「溶かして再鋳造するような変容プロセス」が進んでいることを示すことがあります。' },
];

export const metadata = buildColumnMetadata('fire-dream');
...
export default function FireDreamPage() {
  return (
    <ColumnArticleShell
      slug="fire-dream"
      heroTitle="火の夢の意味とは"
      heroSubtitle="情熱・変容・破壊と再生——炎が映す深層心理"
      faqs={FAQS}
      references={[
        <>C.G. Jung, <em>Psychology and Alchemy</em>, Princeton University Press</>,
        <>C.G. Jung, <em>Dreams</em>, Princeton University Press</>,
      ]}
      nextColumn={{
        href: '/column/water-dream',
        emoji: '🌊',
        heading: '水の夢・溺れる夢も気になりますか？',
        sub: '感情と無意識——水が映す深層心理',
      }}
      cta={{
        title: '火の夢をより詳しく分析したい方へ',
        description: '夢の詳細をAIに伝えると、あなたの深層心理をより具体的に読み解きます。火の大きさ・場所・感じた感情も含めて入力してみてください。',
      }}
    >
