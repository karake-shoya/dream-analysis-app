import { PenLine, BookOpen, Brain, Star } from 'lucide-react';
import ColumnArticleShell from '@/components/column/ColumnArticleShell';
import InContentAd from '@/components/InContentAd';
import { buildColumnMetadata } from '@/lib/seo';

const FAQS = [
  { q: '同じ夢を何度も見るのはなぜですか？', a: 'ユング心理学では、繰り返し見る夢は「無意識が未解決のテーマを意識に届けようとしているサイン」として解釈されます。感情的に解消されていない体験や避け続けている問題が夢として繰り返されます。夢日記でそのテーマを記録・言語化することが、繰り返しの夢を止める第一歩になることがあります。' },
  { q: '夢日記はどんなノートに書けばいいですか？', a: '形式は自由です。枕元に置けるサイズのノートとペン、またはスマートフォンのメモアプリで構いません。大切なのは「起きた直後にすぐ書ける環境」を整えることです。筆記は記憶定着に効果的とされていますが、続けることを最優先に、自分に合った方法を選んでください。' },
  { q: '夢を何も覚えていない日はどうすればいいですか？', a: '覚えていない日も「夢なし（想起不可）」と一言書くことをおすすめします。書けない日をつくらないことが習慣を保つコツです。また、「なんとなく怖い感じがした」「誰かといた気がする」という断片も記録する価値があります。' },
  { q: '夢日記の効果が出るまでどのくらいかかりますか？', a: '個人差はありますが、毎日続けた場合、2〜3週間で夢の想起量が増えてくる人が多いです。1か月ほど経つと繰り返しのテーマやパターンに気づき始め、自己理解に活かせる段階に入ります。' },
  { q: '夢日記をつけると悪夢が増えることはありますか？', a: '夢日記が悪夢の頻度を増やすわけではありません。今まで忘れていた夢を覚えるようになるため、悪夢の記憶も残りやすくなります。悪夢が続いて困る場合は、記録だけにとどめ、内容の解釈は時間をおいて行うとよいでしょう。' },
  { q: '夢日記をつけると明晰夢が見やすくなりますか？', a: 'はい、夢日記は明晰夢の準備として最も重要なステップです。夢を覚える習慣が夢への意識を高め、夢の中で「これは夢だ」と気づく確率が上がります。明晰夢研究者のスティーブン・ラバージをはじめ、多くの実践者が夢日記を明晰夢の必須準備として挙げています。' },
];

export const metadata = buildColumnMetadata('dream-diary');

export default function DreamDiaryPage() {
  return (
    <ColumnArticleShell
      slug="dream-diary"
      heroTitle="夢日記の始め方と効果的な続け方"
      heroSubtitle="夢を書き留めることで、自分の内面が少しずつ見えてくる"
      faqs={FAQS}
      faqVariant="qa"
      references={[
        <>C.G. Jung, <em>Dreams</em>, Princeton University Press</>,
        <>C.G. Jung, <em>Memories, Dreams, Reflections</em>（日本語版：『ユング自伝』みすず書房）</>,
        <>Stephen LaBerge, <em>Lucid Dreaming</em>, Ballantine Books</>,
        <>Calvin S. Hall, <em>The Meaning of Dreams</em></>,
      ]}
      disclaimer="※本ページの内容は、心理学的な一般知識をもとにした情報提供を目的としており、医学的な診断・治療を行うものではありません。心の不調が続く場合は、専門家にご相談ください。"
      nextColumn={{
        href: '/sleeping-positions',
        emoji: '💑',
        heading: '眠っている間の深層心理も確認しませんか？',
        sub: 'カップルの寝る体勢でわかる深層心理 ─ 15パターン＋相性診断',
        eyebrow: '関連コンテンツ',
        accent: 'pink',
      }}
      cta={{
    ...
    >
      ...
      {/* 要点（結論を先に示し、書き方を探しに来た読者の離脱を防ぐ） */}
      <div className="p-6 md:p-8 rounded-3xl bg-purple-900/20 border border-purple-500/20">
        <h2 className="text-xl md:text-2xl font-bold text-white mb-4 flex items-center">
          <Star className="w-6 h-6 mr-2 text-purple-400 shrink-0" />
          夢日記の書き方——まずはこの3ステップ
        </h2>
        <ol className="text-gray-300 text-lg leading-relaxed space-y-2 pl-5 list-decimal">
          <li><span className="text-purple-300 font-bold">枕元にノートかスマホを置く</span>——起きてすぐ書ける環境が最優先</li>
          <li><span className="text-purple-300 font-bold">目覚めたら動く前に書く</span>——日付・場所・登場人物・自分の行動・感情の5つを思い出せる範囲で</li>
          <li><span className="text-purple-300 font-bold">解釈は夜か週末に回す</span>——朝は記録だけに集中する</li>
        </ol>
        <p className="text-gray-400 mt-4">
          2〜3週間続けると夢を覚えている量が増え、1か月ほどで繰り返し現れるテーマに気づけるようになります。
        </p>
      </div>
