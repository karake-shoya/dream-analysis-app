import { Brain, Eye, Zap, CheckCircle, Infinity } from 'lucide-react';
import ColumnArticleShell from '@/components/column/ColumnArticleShell';
import InContentAd from '@/components/InContentAd';
import { buildColumnMetadata } from '@/lib/seo';

const FAQS = [
  { q: '正夢とは何ですか？', a: '正夢（まさゆめ）とは、夢で見た出来事がその後現実に起こる現象のことです。心理学的には、脳の予測機能・無意識の洞察・選択的記憶などのメカニズムによって説明されます。ユング心理学では「共時性（シンクロニシティ）」という概念でも捉えられます。' },
  { q: '正夢は本当にあるのですか？', a: '科学的に「予知能力」として証明された事例はありませんが、脳が過去の情報をもとに未来を予測したり、無意識が微細なサインを拾うことで夢が現実と一致することはあります。体験として感じること自体は本物です。' },
  { q: 'なぜ正夢を見るのですか？', a: '主な理由は3つあります。①脳が睡眠中に情報を整理して未来をシミュレーションする、②無意識が日常の微細なサインをキャッチして夢に反映させる、③「当たった夢」だけが印象に残る選択的記憶の働きです。' },
  { q: '正夢になりやすくする方法はありますか？', a: '夢日記をつけて夢を意識する習慣をつける、就寝前に意識したいテーマを明確に思い描く（認知的プライミング）、睡眠の質を上げてレム睡眠を増やす、感情と記憶の統合を深めて夢の定着率を高めるといった方法が効果的とされています。' },
  { q: '正夢の意味は何ですか？', a: '正夢は、あなたの脳が環境の変化を感じ取っているサイン、または無意識が現実への準備を整えていたと考えられます。ユング心理学では「共時性（意味のある偶然の一致）」として捉えることもできます。直感や観察力が鋭くなっているタイミングとして受け取ることができます。' },
];

export const metadata = buildColumnMetadata('prophetic-dream');
...
export default function PropheticDreamPage() {
  return (
    <ColumnArticleShell
      ...
      heroTitle="正夢とは？"
      heroSubtitle="本当にある？なる方法まで心理学で解説"
      faqs={FAQS}
      faqVariant="qa"
      disclaimer="※本ページで扱う「正夢」は、心理的な体験や記憶の解釈として語られる概念であり、未来を予測・保証するものではありません。"
      disclaimerClassName="mt-12"
    >
      {/* 導入 */}
