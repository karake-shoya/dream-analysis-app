...
interface ColumnArticleMetaProps {
  /** コラムのスラッグ。指定すると公開日・更新日と Article 構造化データを自動で出力する */
  slug?: string;
  /** レジストリを使わずに日付を直接指定する場合 */
  publishedAt?: string;
  updatedAt?: string;
}

export default function ColumnArticleMeta({
  slug,
  publishedAt: publishedAtProp,
  updatedAt: updatedAtProp,
}: ColumnArticleMetaProps) {
  const article = slug ? getColumnArticle(slug) : undefined;

  const publishedAt = publishedAtProp ?? article?.publishedAt;
  const updatedAt = updatedAtProp ?? article?.updatedAt;

  if (!publishedAt) return null;

  const formatDate = (dateStr: string) =>
    dateStr.replace(/-/g, '年').replace(/年(\d+)年/, '年$1月') + '日';
